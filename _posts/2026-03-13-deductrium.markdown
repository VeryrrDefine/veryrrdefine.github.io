...
概括元定理说的是只要条件和结果里没有 z$#0，直接就可以V$#0:xxx，是a6的变式。
...
首先我们可以看到$0 < $0(这里的<是包含)。
第一次你可能会获得#nf($x,$0) < #nf($x,$0)这个神秘表达式，局部变量就是让#nf暂时“失效”，#nf($0,#x)会直接变成$0。我们就可以顺利得到$0<$0了,+30(9834+501)。

接下来是(x < y) > (y< z) > (x< z)。
利用前面两个可以推出(V#x: ((#x @ x) > (#x @ y)))和(V#x: ((#x @ y) > (#x @ z))),用v.t,+50(9884+501)

接下来是x < y, y < x 则x=y。
我们可以得到(Vz: ((z @ x) > (z @ y)))和(Vz: ((z @ y) > (z @ x)))，得到(Vz: ((z @ x) <> (z @ y)))，可以应用( ⊢ (Vx: (Vy: ((Vz: ((z @ x) <> (z @ y))) > (x = y)))))。
